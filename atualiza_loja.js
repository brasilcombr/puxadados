...
<div class="wrap">
  <header>
    <a href="https://brasilcombr.github.io/puxadados/">← Voltar para Loja</a>
  </header>
  <div class="card">
    <div class="left">
      <div class="stepper">
        <div class="step active" data-step="1"><div class="dot">1</div>Dados</div>
        <div class="step" data-step="2"><div class="dot">2</div>Endereço</div>
      ...
      <div id="stepContent">
        <section data-step="1" class="step-section">
          ...
          <div style="margin-top:18px"><button class="btn" onclick="nextStep()">Próximo</button></div>
        </section>
        <section data-step="2" class="step-section" style="display:none">
          <h1>Endereço</h1>
          <div><label>Rua</label><input id="rua"></div>
          <div class="grid"><div><label>Número</label><input id="num"></div><div><label>Bairro</label><input id="bairro"></div></div>
        ...
        <section data-step="3" class="step-section" style="display:none">
          <h1>Pagamento</h1>
          <p class="subtitle">Frete grátis aplicado automaticamente.</p>
          <div class="pay-grid">
            <div class="pay-opt" data-pay="pix" onclick="selectPay(this)"><div class="pay-icon">⚡</div><div><strong>Pix (5% OFF)</strong></div></div>
            <div class="pay-opt" data-pay="cartao" onclick="selectPay(this)"><div class="pay-icon">💳</div><div><strong>Cartão</strong></div></div>
          </div>
    ...
    <aside class="right">
      <div>
        <div class="summary-title" id="prodName">Produto</div>
        <div class="product-row">
          <img id="prodThumb" class="thumb" src="../img/produto.png">
          <div><div class="price" id="prodPrice">R$ 0,00</div></div>
        </div>
      </div>
    </aside>
  </div>
...
function selectPay(el){document.querySelectorAll('.pay-opt').forEach(x=>x.classList.remove('selected'));el.classList.add('selected');selectedPay=el.dataset.pay;document.getElementById('payFormArea').innerHTML=selectedPay==='cartao'?"<p>Formulário de cartão aqui...</p>":"<p>Use o QR Pix gerado.</p>";}
...
/* Produto injetado */
document.getElementById("prodName").innerText = "${nome}";
document.getElementById("prodPrice").innerText = "${preco}";
document.getElementById("prodThumb").src = "../img/${img}";
</script>`);
  fs.writeFileSync(checkoutFile, checkoutHtml, "utf8");
  console.log("✔ Checkout atualizado:", checkoutFile);

  // corrige botão no produto
...
try {
  execSync("git add . && git commit -m 'Atualiza checkouts' && git push origin main --force", {stdio:"inherit"});
